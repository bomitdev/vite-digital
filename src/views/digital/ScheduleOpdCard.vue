  ...
  <div class="container-fluid">
    <div class="row">
      <div>
        ...
        <h3>ตารางเวรห้องบัตร : เดือน {{ months[selectedMonth - 1] }}</h3>
      </div>
      ...
      <button class="btn" @click="goToFromDuties">+</button>
      <button class="btn" @click="goToFromDutiesOPDcard">-</button>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    async fetchSchedule() {
      try {
        ...
        const response = await fetch(
          `/api-digital/duties_opdcard/get-schedule-opdcard.php?year=${year}&month=${this.selectedMonth}`
        );
    ...
    goToFromDutiesOPDcard() {
      this.$router.push({
        path: '/from-dutyopdcard'
      });
    }
  },
  mounted() {
