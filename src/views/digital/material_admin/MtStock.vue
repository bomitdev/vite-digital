...
export default {
  ...
  methods: {
    async fetchMaterials() {
      try {
        let url = `/api-digital/material_admin/get_materials.php?search=${encodeURIComponent(this.search)}`;
    ...
    async deleteItem(id, name) {
      if (
        ...
        try {
          const res = await axios.post('/api-digital/material_admin/delete_material.php', { id });
          if (res.data.status === 'success') {
            this.fetchMaterials();
          } else {
            alert(res.data.message);
