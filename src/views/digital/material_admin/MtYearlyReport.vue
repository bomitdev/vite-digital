      ...
      <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
          ...
          <h2 class="fw-bold text-dark mb-0">
            <i class="bi bi-file-earmark-pdf me-2 text-danger"></i>รายงานตามช่วงเวลา
            วัสดุงานบริหารทั่วไป (PDF)
          </h2>
        </div>
        <div>
          ...
          <div class="card border-0 shadow-sm rounded-4 h-100 bg-white" style="min-height: 600px">
            ...
            >
              ...
              <p class="text-secondary" style="max-width: 400px">
                ...
                วัสดุงานบริหารทั่วไป ตามช่วงเวลาที่คุณเลือก คลิกปุ่ม
  ...
  methods: {
    ...
    async fetchData() {
      ...
      try {
        ...
        const res = await axios.get(
          `/api-digital/material_admin/get_yearly_toner_summary.php?start_date=${start}&end_date=${end}`
        );
    ...
    async generatePDF() {
      ...
      try {
        ...
        doc.setFont('Sarabun', 'normal');
        doc.text('กลุ่มงานบริหารทั่วไป โรงพยาบาลชานุมาน อำเภอชานุมาน จังหวัดอำนาจเจริญ', 42, 40);
        ...
        doc.setFont('Sarabun', 'normal');
        doc.text('หัวหน้ากลุ่มงานบริหารทั่วไป', 32, 64);

        // Paragraph 1
        const p1Opts = { maxWidth: 160, align: 'justify', lineHeightFactor: 1.5 };
        const indentStr = '          ';
        doc.text(
          `${indentStr}กลุ่มงานบริหารทั่วไปได้สรุปยอดการรับ-จ่าย วัสดุงานบริหารทั่วไป ระหว่างวันที่ ${dStart} ถึง ${dEnd} เพื่อส่งการเงินจัดทำรายงาน ดังรายการต่อไปนี้`,
          20,
          74,
          p1Opts
        );
        ...
        // Signature 1
        let signY = listY + 65;
        doc.text('(นางภรภัค  อรอินทร์)', 140, signY, { align: 'center' });
        doc.line(125, signY + 1, 155, signY + 1);
        doc.text('เจ้าพนักงานพัสดุ', 140, signY + 8, { align: 'center' });
        ...
        // Signature 2
        signY += 30;
        doc.text('(นางสาวศิริลักษญ์  พรหมสาขา ณ สกลนคร)', 75, signY, { align: 'center' });
        doc.line(55, signY + 1, 95, signY + 1);
        doc.text('นักวิชาการพัสดุ', 75, signY + 8, { align: 'center' });
        ...
        autoTable(doc, {
          ...
          didDrawPage: (data) => {
            doc.setFontSize(16);
            doc.setFont('Sarabun', 'bold');
            doc.text(`การใช้วัสดุงานบริหารทั่วไป ระหว่างวันที่ ${dStart} ถึง ${dEnd}`, 148, 15, {
              align: 'center'
            });
        ...
        doc.text('(นางภรภัค  อรอินทร์)', 105, sumSignY + 10, { align: 'center' });
        doc.text('เจ้าพนักงานพัสดุ', 105, sumSignY + 18, { align: 'center' });
        ...
        doc.text('(นางสาวศิริลักษญ์  พรหมสาขา ณ สกลนคร)', 105, sumSignY + 22, { align: 'center' });
        doc.text('นักวิชาการพัสดุ', 105, sumSignY + 30, { align: 'center' });
        ...
        doc.save(`custom-admin-material-report-${this.startDate}-to-${this.endDate}.pdf`);
        Swal.close();
      } catch (err) {
        console.error(err);
