...
export default {
  ...
  methods: {
    ...
    async fetchItUsers() {
      try {
        const res = await axios.get('/api-digital/material_admin/get_it_users.php');
        if (res.data.status === 'success') {
    ...
    async submitTx() {
      ...
      try {
        const res = await axios.post('/api-digital/material_admin/transaction_out.php', this.form);
        if (res.data.status === 'success') {
