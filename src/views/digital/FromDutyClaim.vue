  ...
  <div class="container mt-5">
    <div class="card shadow-sm">
      <div class="card-header bg-primary text-white">
        <h2 class="mb-0">
          <i class="bi bi-calendar-plus me-2"></i>เพิ่มตารางเวรงานประกัน/เบิกเครม
        </h2>
      </div>
      <div class="card-body">
        <form @submit.prevent="submitForm">
          ...
          <div class="mb-4">
            ...
            <select
              v-model="form.employees_claim_id"
...
export default {
  ...
  methods: {
    async fetchEmployees() {
      try {
        const response = await axios.get('/api-digital/duties_claim/get_employee_claim.php');
    ...
    async submitForm() {
      if (!this.form.employees_claim_id || this.form.dates.some((date) => !date)) {
      ...
      try {
        const response = await axios.post(
          '/api-digital/duties_claim/add_duties_claim.php',
          this.form
        );
        this.message = response.data.message || 'บันทึกข้อมูลสำเร็จ';
        this.form.employees_claim_id = '';
