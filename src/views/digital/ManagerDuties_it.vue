<template>
  <div class="container mt-5">
    <div class="d-flex justify-content-between align-items-center mb-3">
      <h2>จัดการตารางเวรคอมพิวเตอร์</h2>
    ...
    <table class="table table-bordered table-hover mt-3">
      ...
      <tbody>
        <tr v-for="duty in duties" :key="duty.id">
          ...
          <td>{{ duty.name }}</td>
          <td>
            <span v-if="duty.rate_override" class="badge bg-primary">{{ duty.rate_override }}</span>
            <span v-else-if="duty.is_special == 1" class="badge bg-warning text-dark">เรทพิเศษ</span>
            <span v-else-if="duty.is_special == 2" class="badge bg-info text-dark">เรทวันหยุด 2 เท่า</span>
            <span v-else class="text-muted">เรทปกติ</span>
          </td>
          <td>
...
export default {
  name: 'DutyManager',
  data() {
    // สร้าง Object วันที่ปัจจุบันขึ้นมารอไว้ก่อน
    ...
    return {
      ...
      // --- จุดสำคัญ: กำหนดให้เป็นเดือนปัจจุบันเสมอ ---
      // getMonth() จะได้ค่า 0-11 เราจึงต้อง +1 ให้เป็น 1-12
      selectedMonth: today.getMonth() + 1,

      // (แถม) ควรระบุปีด้วย ไม่งั้นข้อมูลเดือน 12 ปีที่แล้วจะปนมา
      ...
      apiUrl: '/api-digital/duties/crud_duties_it.php'
    };
  },
  mounted() {
    // ทันทีที่หน้าเว็บโหลดเสร็จ จะเรียกฟังก์ชันนี้
    // ซึ่งมันจะใช้ค่า selectedMonth (เดือนปัจจุบัน) ที่เราตั้งไว้ข้างบนส่งไปหา PHP ทันที
    this.fetchDuties();
    this.fetchEmployees();
  },
  methods: {
    fetchDuties() {
      // ส่งทั้งเดือนและปีไปที่ API
      axios
        .get(
    ...
    // ... (Functions อื่นๆ: fetchEmployees, openEditModal, updateDuty, formatDate เหมือนเดิม) ...
    fetchEmployees() {
      axios
      ...
      axios
        ...
        .then((response) => {
          if (response.data.message === 'Update employee successful') {
            // flash success briefly or just alert
...
<style scoped>
/* Scoped CSS จะมีผลเฉพาะ component นี้ */
.modal.d-block {
  z-index: 1050; /* ให้แน่ใจว่า Modal อยู่บนสุด */
}
</style>
