  ...
  <div class="report-container pt-3 pb-5 bg-light min-vh-100">
        ...
        <div class="card-body">
          <div class="d-flex justify-content-between align-items-center mb-3">
            <h2 class="h4 mb-0 text-purple fw-bold">
              <i class="bi bi-file-earmark-spreadsheet me-2"></i> ระบบรายงานเบิกจ่าย OT
              (งานประกัน/เบิกเครม)
            </h2>
            <button
    ...
    >
      <div class="text-center mb-1">
        <h5 class="fw-bold fs-16pt">
          ตารางการเวรขออนุมัติปฏิบัติงานนอกเวลาราชการ งานประกัน/เบิกจ่าย โรงพยาบาลชานุมาน
    ...
    >
      <div class="text-center mb-3">
        <h5 class="fw-bold fs-16pt">
          หลักฐานการเบิกจ่ายเงินค่าตอบแทนนอกเวลาราชการ งานประกัน/เบิกจ่าย โรงพยาบาลชานุมาน
          <span v-if="isPrintDouble">(2 เท่า)</span>
        </h5>
        <h6 class="mt-1 fs-16pt">
        ...
        <thead>
          <tr style="font-size: 12pt">
            ...
            <th :colspan="daysInMonth" class="text-center">
              วันที่ปฏิบัติงานล่วงเวลา
            </th>
            <th rowspan="2" class="align-middle" style="width: 35px; font-size: 10pt; line-height: 1;">รวม<br />วัน</th>
            <th rowspan="2" class="align-middle" style="width: 50px; font-size: 10pt; line-height: 1;">จำนวน<br />เงิน</th>
            ...
            <th rowspan="2" class="align-middle" style="width: 40px; font-size: 10pt;">หมายเหตุ</th>
          </tr>
          <tr>
            <th
              v-for="d in daysInMonth"
          ...
          <template v-for="(emp, index) in filteredReportData" :key="emp.id">
            <tr v-for="(breakdown, bIdx) in emp.rate_breakdowns" :key="emp.id + '-' + bIdx">
                ...
                <td
                  ...
                  style="padding: 0"
                >
                  <template v-if="emp.duties[d] === 'CLAIM' && (breakdown.is_special ? (emp.duties_rate_double && emp.duties_rate_double[d] == breakdown.rate) : (emp.duties_rate_normal ? emp.duties_rate_normal[d] == breakdown.rate : emp.duties_rate[d] == breakdown.rate))">
                     {{ monthDaysInfo[d] }}
                  </template>
                </td>
    ...
    >
      <div class="text-center mb-1">
        <h5 class="fw-bold fs-16pt">
          ตารางการเวรปฏิบัติงานนอกเวลาราชการ งานประกัน/เบิกจ่าย โรงพยาบาลชานุมาน
          <span v-if="isPrintDouble">(2 เท่า)</span>
        </h5>
        <h6 class="mt-1 fs-16pt">
          ประจำเดือน {{ monthNames[selectedMonth - 1] }} {{ selectedYear + 543 }}
        ...
        <tbody>
          <tr v-for="(emp, index) in filteredReportData" :key="emp.id">
            ...
            <td
              ...
              :key="d"
              :class="{ 'bg-holiday': monthDaysInfo[d] === 'O' }"
              style="padding: 0; font-size: 14pt"
            >
              <template v-if="emp.duties[d] === 'CLAIM'">
    ...
    >
      ...
      <div class="memo-metadata">
        ...
        <div class="d-flex align-items-baseline mb-0">
          ...
          <span class="fs-16pt">ขออนุมัติปฏิบัติงานนอกเวลาราชการ (เวรส่งเบิกเคลม)</span>
        </div>
      ...
      <div class="memo-body fs-16pt">
        <div class="content-paragraph text-justify">
          ด้วยกลุ่มงานสุขภาพดิจิทัล โรงพยาบาลชานุมาน รับผิดชอบการเบิกค่ารักษาพยาบาล ทั้งผู้ป่วยนอก ,
          ผู้ป่วยใน และสแกนเวชระเบียนผู้ป่วยใน รวมถึงการปฏิบัติงานตามนโยบายเร่งด่วนในแต่ละห้วงเวลา
          เพื่อให้การดำเนินการส่งเบิกค่ารักษาพยาบาลดังกล่าว ส่งได้ทันตามกำหนดเวลา
        </div>
        <div class="content-paragraph text-justify">
    ...
    >
      ...
      <div class="mb-3 fs-16pt">
        ...
        <div>จุดปฏิบัติงาน กลุ่มงานสุขภาพดิจิทัล (งานประกัน/เบิกจ่าย)</div>
      </div>
...
export default {
  ...
  methods: {
    async fetchData() {
      this.isLoading = true;
      try {
        const response = await axios.get('/api-digital/duties_claim/get-ot-report-claim.php', {
