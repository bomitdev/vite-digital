...
try {
    ...
    $stmtMaterial = $pdo2->prepare("SELECT id, name, balance, price_per_unit FROM mt_materials WHERE id = :id FOR UPDATE");
    ...
    $stmtTx = $pdo2->prepare("
        INSERT INTO mt_transactions (material_id, action_type, quantity, total_price, action_date, user_profile_name, receiver_name, reference_dest, note)
    ...
    $stmtUpdate = $pdo2->prepare("UPDATE mt_materials SET balance = :balance WHERE id = :id");
