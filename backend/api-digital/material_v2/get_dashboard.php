...
try {
    ...
    $stmtTotalMaterials = $pdo2->query("SELECT COUNT(*) AS count FROM mt_materials");
    $total_sku = $stmtTotalMaterials->fetch()['count'];

    // 2. สรุปรายการวัสดุที่ยอดคงเหลือต่ำกว่าเกณฑ์การแจ้งเตือน
    $stmtLowStock = $pdo2->query("SELECT COUNT(*) AS count FROM mt_materials WHERE balance <= min_alert");
