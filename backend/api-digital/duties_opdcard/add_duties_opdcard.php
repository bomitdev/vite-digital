...
try {
    ...
    if (!isset($data['employees_opdcard_id']) || !isset($data['dates']) || !is_array($data['dates'])) {
        throw new Exception('ข้อมูลไม่ครบถ้วน');
    }

    $employee_id = $data['employees_opdcard_id'];
    $dates = $data['dates'];

    $stmt = $pdo2->prepare("INSERT INTO duties_opdcard(employees_opdcard_id, date, rate_override, is_special) VALUES (:employees_opdcard_id, :date, :rate_override, :is_special)");
    ...
    foreach ($dates as $dateObj) {
        ...
        if ($stmt->execute([
            ':employees_opdcard_id' => $employee_id,
            ':date' => $dateStr,
            ':rate_override' => $rate,
