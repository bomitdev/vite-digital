...
try {
    ...
    $stmtTx = $pdo2->prepare("SELECT material_id, action_type, quantity FROM mt_admin_transactions WHERE id = :id FOR UPDATE");
    ...
    $stmtMat = $pdo2->prepare("SELECT balance FROM mt_admin_materials WHERE id = :id FOR UPDATE");
    ...
    $stmtUpdateMat = $pdo2->prepare("UPDATE mt_admin_materials SET balance = :balance, updated_at = NOW() WHERE id = :id");
    ...
    $stmtDeleteTx = $pdo2->prepare("DELETE FROM mt_admin_transactions WHERE id = :id");
